...
bash "create aliases for #{ruby_version}" do
  code "/usr/local/rvm/bin/rvm alias create default #{ruby_version}"
end

bash "Adding rvm env to profile" do
  code "echo source /usr/local/rvm/scripts/rvm >> /etc/profile"
  not_if "cat /etc/profile | grep rvm"
end
